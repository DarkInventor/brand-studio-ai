...
// Add a type for the Instagram profile
...
export default function SchedulerPage() {
  ...
  async function fetchBrandKitInstagramProfile(brandKitId: string) {
    ...
    const { data: kit, error } = await supabase
      .from("brand_kits")
      .select("instagram_username, instagram_name, instagram_profile_picture_url")
      .eq("id", brandKitId as any)
      .single()
    if (!error && kit && (kit as any).instagram_username) {
      setInstagramProfile(kit as InstagramProfile)
    } else {
  ...
  // Fetch Instagram profile when brand kit changes
  useEffect(() => {
    if (selectedBrandKitId) {
      fetchBrandKitInstagramProfile(selectedBrandKitId)
    }
  }, [selectedBrandKitId])

  // After redirect, call API route and then refetch profile
  useEffect(() => {
    if (typeof window !== "undefined") {
      const params = new URLSearchParams(window.location.search.substring(1))
      const code = params.get("code")
      const brandKitId = params.get("brandKitId") || selectedBrandKitId
      if (code && brandKitId) {
        fetch(`/api/instagram/callback?code=${code}&brandKitId=${brandKitId}`)
          .then(() => fetchBrandKitInstagramProfile(brandKitId))
          .then(() => window.history.replaceState({}, document.title, "/scheduler"))
      }
    }
  }, [selectedBrandKitId])
  ...
  async function handleConnectInstagram() {
    ...
    // Only pass selectedBrandKitId if signInWithInstagram expects it
  ...
  async function handlePublishToInstagram() {
    if (!previewPost || !selectedBrandKitId) return
    // Call backend API to publish post and update status
    const res = await fetch("/api/instagram/publish", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ postId: previewPost.id, brandKitId: selectedBrandKitId }),
    })
    if (res.ok) {
      toast({ title: "Post published!", description: "Your post was published to Instagram." })
      closePreview()
      fetchAndSync(selectedBrandKitId, currentDate)
    } else {
      toast({ title: "Error publishing post", description: "Failed to publish to Instagram.", variant: "destructive" })
    }
  }
  ...
  useEffect(() => {
    async function loadBrandKits() {
      ...
      try {
        ...
        const validKits: BrandKit[] = (kits.filter((kit) => !(kit as any).error) as unknown) as BrandKit[]
        setBrandKits(validKits)

        // If we have a stored brand kit ID, check if it exists in the loaded kits
        if (selectedBrandKitId) {
          const kitExists = validKits.some((kit) => typeof kit === 'object' && 'id' in kit && typeof kit.id === 'string')
          if (!kitExists && validKits.length > 0) {
            // If the stored kit doesn't exist anymore, use the first available kit
            const firstKit = validKits.find((kit) => typeof kit === 'object' && 'id' in kit && typeof kit.id === 'string')
            if (firstKit && typeof (firstKit as any).id === 'string') {
              setSelectedBrandKitId((firstKit as any).id)
              localStorage.setItem("selectedBrandKitId", (firstKit as any).id)
            }
          }
        } else if (validKits.length > 0) {
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              {loadingBrandKits ? (
                ...
                >
                  <SelectTrigger className="w-full sm:w-[180px]">
                    <SelectValue placeholder={selectedBrandKitId !== null ? selectedBrandKitId : undefined} />
                  </SelectTrigger>
                  <SelectContent>
                    {brandKits.length === 0 ? (
            ...
            <div className="flex items-center gap-1 sm:gap-3">
              ...
              {isLoadingInstagram ? (
                ...
                <div className="flex items-center gap-2">
                  <img src={instagramProfile.instagram_profile_picture_url || "/placeholder.svg"} alt="Instagram" className="h-8 w-8 rounded-full" />
                  <span>{instagramProfile.instagram_username || instagramProfile.instagram_name}</span>
                  <Button onClick={() => openPreview(unscheduledPosts[0])} className="ml-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-xs sm:text-sm">
        ...
        <DialogContent>
          ...
          {previewPost && (
            <div className="flex flex-col items-center gap-4">
              <img src={previewPost.image_url || "/placeholder.svg"} alt="Preview" className="w-48 h-48 object-cover rounded" />
              <p className="text-center">{previewPost.caption}</p>
            </div>
          )}
          <DialogFooter>
            <Button onClick={handlePublishToInstagram} disabled={!previewPost} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700">
              Post to Instagram
            </Button>
          </DialogFooter>
        </DialogContent>
