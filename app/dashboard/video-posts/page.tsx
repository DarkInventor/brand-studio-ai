...
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Clock, ImageIcon, LucideFrame, Maximize2, Palette, Sparkles } from "lucide-react"
import { Textarea } from "@/components/ui/textarea"
  ...
  return (
    <div className="bg-white text-gray-900 min-h-screen">
      <div className="container mx-auto p-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="flex items-center gap-1.5 rounded-full text-xs bg-white/50 backdrop-blur-sm w-full sm:w-auto min-w-[120px]"
              >
                <Palette className="w-4 h-4" />
                <span className="sm:inline truncate">
                  {(Array.isArray(brandKits) && brandKits.find((bk) => bk?.id === selectedBrandKit)?.name) ||
                    "Brand Kit"}
                </span>
              </Button>
            </DropdownMenuTrigger>
        ...
        {/* Video Posts Grid - Masonry Layout */}
        {isLoading ? (
          <div className="flex justify-center items-center h-64">
            <Loader2 className="h-10 w-10 animate-spin text-primary" />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {Array.isArray(videoPosts) &&
              videoPosts.map((post, index) => {
                // Determine grid span based on index for a more dynamic layout
                ...
                return (
                  <div
                    key={post?.id}
                    className={`${isWide ? "sm:col-span-2" : ""} ${isTall ? "row-span-2" : ""} ${isLarge ? "sm:col-span-2 row-span-2" : ""}`}
                  >
                    <Card className="group overflow-hidden border border-border/50 bg-card transition-all duration-300 hover:shadow-lg hover:border-primary/20 hover:translate-y-[-4px] h-full">
                      <CardContent className="p-0 relative">
                        <div
                          className={`relative overflow-hidden bg-muted border rounded-none shadow-none ${
                        ...
                        >
                          {post?.video_url ? (
                            <div className="w-full h-full border rounded-none shadow-none">
                              <Player
                                component={VideoComposition}
                                durationInFrames={30 * (post.video_duration || 5)}
                                fps={30}
                      ...
                      <CardFooter className={`p-4 ${isLarge ? "bg-primary/5" : ""}`}>
                        <div className="space-y-2 w-full">
                          <p
                            className={`text-sm ${isLarge ? "text-base font-medium" : ""} text-muted-foreground line-clamp-2`}
                          >
                            {post?.caption}
                          </p>
                          <div className="flex items-center justify-between">
                            <Badge variant={isLarge ? "default" : "outline"} className="text-xs px-2 py-0">
                              Video
                            </Badge>
        ...
        {/* Remotion Timeline Editor Modal */}
        <Dialog
          open={showTimeline && !!editingVideo}
          onOpenChange={(open) => {
            if (!open) {
              setShowTimeline(false)
        ...
        <div className="fixed bottom-4 left-0 sm:left-[240px] right-0 bg-white/70 backdrop-blur-md border rounded-lg border-gray-200/50 p-1.5 sm:p-2 max-w-5xl mx-auto shadow-lg ">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1.5 border-b border-gray-200/50 pb-2 sm:pb-0 ">
            <div className="flex flex-wrap items-center gap-1.5 mb-2">
              <Button
                variant="outline"
                size="sm"
                className="flex items-center gap-1.5 rounded-full text-xs bg-white/50 backdrop-blur-sm"
              >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  ...
                  <DropdownMenuItem onSelect={() => setSelectedEffect("None")}>None</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Let's YMCA!")}>Let's YMCA!</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Subject 3 Fever")}>
                    Subject 3 Fever
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Ghibli Live!")}>Ghibli Live!</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Suit Swagger")}>Suit Swagger</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Muscle Surge")}>Muscle Surge</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("360° Microwave")}>
                    360° Microwave
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Warmth of Jesus")}>
                    Warmth of Jesus
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Emergency Beat")}>
                    Emergency Beat
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Anything, Robot")}>
                    Anything, Robot
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Kungfu Club")}>Kungfu Club</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Mint in Box")}>Mint in Box</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Retro Anime Pop")}>
                    Retro Anime Pop
                  </DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Vogue Walk")}>Vogue Walk</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => setSelectedEffect("Mega Dive")}>Mega Dive</DropdownMenuItem>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-1.5 gap-1.5">
            ...
            >
              ...
              <span className="ml-1 text-xs bg-white/20 px-1 py-0.5 rounded">⌘ 30</span>
            </Button>
          </div>
          {error && <div className="text-red-500 text-xs mt-2">{error}</div>}
        </div>
