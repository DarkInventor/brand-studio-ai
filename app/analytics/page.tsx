...
export default function AnalyticsPage() {
  ...
  async function fetchData(showRefreshIndicator = false) {
    if (showRefreshIndicator) {
      ...
      // Filter out error objects for posts and brand kits
      const postsRows = (postsData ?? []).filter((x: any): x is Post => x && typeof x === 'object' && 'id' in x && 'updated_at' in x)
      const brandKitsRows = (brandKitsData ?? []).filter((x: any): x is BrandKit => x && typeof x === 'object' && 'id' in x && 'name' in x)
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          {feedback.contentSuggestions && feedback.contentSuggestions.length > 0 ? (
            <ul className="space-y-3">
              {feedback.contentSuggestions.map((item: string | { suggestion: string; rationale?: string; exampleImplementation?: string }, index: number) => (
                <li key={index} className="flex gap-2">
                  <CheckCircle2 className="h-5 w-5 text-green-600 flex-shrink-0 mt-0.5" />
                  <span className="text-sm">{typeof item === "string" ? item : isAISuggestionObject(item) ? item.suggestion : ""}</span>
                </li>
              ))}
            </ul>
          ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className={`border border-gray-100 bg-white ${isHistorical ? "opacity-90" : ""}`}>
          ...
          <CardContent>
            ...
            {/* Brand-specific feedback if available */}
            {feedback.brandConsistency?.brandSpecificFeedback && 
             Object.keys(feedback.brandConsistency.brandSpecificFeedback).length > 0 && (
              <div className="mt-4">
                <h4 className="text-sm font-medium mb-2">Brand-Specific Feedback</h4>
                <div className="space-y-3">
                  {Object.entries(feedback.brandConsistency.brandSpecificFeedback).map(([brandId, brandFeedback]) => (
                    <div key={brandId} className="p-3 rounded-md bg-blue-50">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-sm font-medium text-blue-800">{brandFeedback.name}</span>
                        <Badge className="bg-blue-100 text-blue-800">
                          Score: {brandFeedback.score}/10
                        </Badge>
                      </div>
                      <p className="text-sm text-blue-700">{brandFeedback.feedback}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <Card className={`border border-gray-100 bg-white ${isHistorical ? "opacity-90" : ""}`}>
          ...
          <CardContent>
            ...
            {/* Brand-specific feedback if available */}
            {feedback.captionQuality?.brandSpecificFeedback && 
             Object.keys(feedback.captionQuality.brandSpecificFeedback).length > 0 && (
              <div className="mt-4">
                <h4 className="text-sm font-medium mb-2">Brand-Specific Feedback</h4>
                <div className="space-y-3">
                  {Object.entries(feedback.captionQuality.brandSpecificFeedback).map(([brandId, brandFeedback]) => (
                    <div key={brandId} className="p-3 rounded-md bg-purple-50">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-sm font-medium text-purple-800">{brandFeedback.name}</span>
                        <Badge className="bg-purple-100 text-purple-800">
                          Score: {brandFeedback.score}/10
                        </Badge>
                      </div>
                      <p className="text-sm text-purple-700">{brandFeedback.feedback}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <Card className={`border border-gray-100 bg-white ${isHistorical ? "opacity-90" : ""}`}>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              {feedback.brandKitOptimizations.brandSpecificOptimizations && 
               Object.keys(feedback.brandKitOptimizations.brandSpecificOptimizations).length > 0 && (
                <div className="mt-4">
                  <h4 className="text-sm font-medium mb-2">Brand-Specific Optimizations</h4>
                  <div className="space-y-4">
                    {Object.entries(feedback.brandKitOptimizations.brandSpecificOptimizations).map(([brandId, brandOpt]) => (
                      <Collapsible key={brandId} className="border border-gray-200 rounded-lg">
                        <CollapsibleTrigger className="flex justify-between items-center w-full p-3 text-left">
                          <span className="font-medium">{brandOpt.name}</span>
                          <ChevronDown className="h-4 w-4" />
                        </CollapsibleTrigger>
                        <CollapsibleContent className="p-3 pt-0 space-y-2">
                          {brandOpt.nameRecommendation && (
                            <div className="p-2 rounded bg-blue-50">
                              <span className="text-xs font-medium text-blue-800">Name:</span>
                              <p className="text-sm text-blue-700">{brandOpt.nameRecommendation}</p>
                            </div>
                          )}
                          {brandOpt.descriptionRecommendation && (
                            <div className="p-2 rounded bg-purple-50">
                              <span className="text-xs font-medium text-purple-800">Description:</span>
                              <p className="text-sm text-purple-700">{brandOpt.descriptionRecommendation}</p>
                            </div>
                          )}
                          {brandOpt.toneRecommendation && (
                            <div className="p-2 rounded bg-amber-50">
                              <span className="text-xs font-medium text-amber-800">Tone:</span>
                              <p className="text-sm text-amber-700">{brandOpt.toneRecommendation}</p>
                            </div>
                          )}
                          {brandOpt.colorRecommendation && (
                            <div className="p-2 rounded bg-green-50">
                              <span className="text-xs font-medium text-green-800">Colors:</span>
                              <p className="text-sm text-green-700">{brandOpt.colorRecommendation}</p>
                            </div>
                          )}
                        </CollapsibleContent>
                      </Collapsible>
      ...
      {feedback?.contentCalendar && (
        <Card className={`border border-gray-100 bg-white ${isHistorical ? "opacity-90" : ""}`}>
          ...
          <CardContent>
            <div className="space-y-4">
              {feedback.contentCalendar.recommendedFrequency && (
                <div className="p-3 rounded-md bg-indigo-50 border border-indigo-100">
                  <h4 className="text-sm font-medium text-indigo-900 mb-1">Recommended Posting Frequency</h4>
                  <p className="text-sm text-indigo-800">{feedback.contentCalendar.recommendedFrequency}</p>
                </div>
              )}
              {feedback.contentCalendar.bestTimeToPost && (
                <div className="p-3 rounded-md bg-cyan-50 border border-cyan-100">
                  <h4 className="text-sm font-medium text-cyan-900 mb-1">Best Time to Post</h4>
                  <p className="text-sm text-cyan-800">{feedback.contentCalendar.bestTimeToPost}</p>
                </div>
              )}
              {feedback.contentCalendar.contentIdeas && feedback.contentCalendar.contentIdeas.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium mb-2">Content Ideas</h4>
                  <ul className="space-y-2">
                    {feedback.contentCalendar.contentIdeas.map((idea, index) => (
                      <li key={index} className="flex gap-2 p-2 rounded-md bg-gray-50">
      ...
      {feedback?.competitiveAnalysis && (
        <Card className={`border border-gray-100 bg-white ${isHistorical ? "opacity-90" : ""}`}>
          ...
          <CardContent>
            <div className="p-4 rounded-md bg-gradient-to-r from-green-50 to-teal-50 border border-teal-100">
              <div className="flex items-start gap-3">
                <LineChartIcon className="h-5 w-5 text-teal-600 mt-1" />
                <div>
                  <h4 className="text-sm font-medium text-teal-900 mb-1">Strategic Insights</h4>
