...
//           <CardTitle className="text-2xl font-bold">Enter Your Brand Details</CardTitle>
//         </CardHeader>
//         <CardContent>
//           {error && (
...
export default function BrandKitForm() {
  ...
  useEffect(() => {
    async function checkAuthAndLoadKits() {
      ...
      setBrandKits(kits)
      if (kits.length > 0 && kits[0] && typeof kits[0].id === "string") {
  ...
  useEffect(() => {
    if (!selectedBrandKitId) return
    const kit = brandKits.find(
      (k: any) => k && typeof k === "object" && typeof k.id === "string" && k.id === selectedBrandKitId,
    )
    if (!kit) return
    setPrimaryColor(kit.primary_color || "#7C3AED")
    setSecondaryColor(kit.secondary_color || "#0EA5E9")
